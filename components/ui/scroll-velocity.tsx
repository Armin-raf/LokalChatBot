...
const ScrollVelocity = React.forwardRef<HTMLDivElement, ScrollVelocityProps>(
  ({ children, velocity = 5, movable = true, clamp = false, className, ...props }, ref) => {
    ...
    return (
      ...
      >
        <motion.div
          className="flex flex-row flex-nowrap whitespace-nowrap *:mr-10 *:block"
          style={{ x }}
        >
          {typeof children === "string" ? (
            <>
              {Array.from({ length: 5 }).map((_, idx) => (
                <span key={idx} className="text-xl font-semibold uppercase md:text-2xl xl:text-4xl">{children}</span>
              ))}
            </>
          ) : (
            // Duplicate children to ensure continuous loop
            <>
                {children}
                {children}
            </>
